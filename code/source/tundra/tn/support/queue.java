...
// -----( CREATED: 2014-08-10 17:19:18 EST
...

{
        ...
        throws ServiceException
	{
		...
		try {
		  ...
		  each(queue, service, scope == null? pipeline : scope, concurrency, 0, 0, ordered);
		} finally {
		  cursor.destroy();
		}
		// --- <<IS-END>> ---
	...
	public static void each(String queueName, String service, IData pipeline, int concurrency, int batchLimit, int retryLimit, boolean ordered) throws ServiceException {
	  try {
	    ...
	    if (retryLimit <= 0) retryLimit = Byte.MAX_VALUE;
	    ...
	    if (concurrency <= 1) {
	      ...
	      eachConcurrent(queue, EXECUTE_TASK_SERVICE, service, pipeline, concurrency, batchLimit);
	    }
	...
	protected static void eachConcurrent(com.wm.app.tn.delivery.DeliveryQueue queue, com.wm.lang.ns.NSName executeTaskService, String service, IData pipeline, int concurrency, int batchLimit) throws ServiceException {
	  boolean invokedByTradingNetworks = invokedByTradingNetworks();
	...
	protected static <T> java.util.List<T> awaitAll(java.util.Queue<java.util.concurrent.Future<T>> futures) {
	  java.util.List<T> results = new java.util.ArrayList<T>(futures.size());
	  while(futures.size() > 0) {
	    results.add(awaitOldest(futures));
	  }
	  return results;
	}
	
	// waits for the first/head future in the given queue to complete
	protected static <T> T awaitOldest(java.util.Queue<java.util.concurrent.Future<T>> futures) {
	...
	protected static boolean retry(com.wm.app.tn.delivery.GuaranteedJob job, int retryLimit, boolean ordered) throws java.sql.SQLException, java.io.IOException, ServiceException {
	  ...
	  if (ordered) {
	    ...
	    if (jobLimit > 0) {
	      retry = retries < jobLimit;
	    } else {
	      retry = retries < retryLimit;
	    }
